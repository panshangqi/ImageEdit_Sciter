...
bool CMuPDFConvert::Pdf2Png(const wchar_t* wcharPdfFile, const char* imageDir, const char* imageName, int &pageNum)
{
	char tempPath[1024];
	...
	//sprintf(savePath, "%s\\%s.png",imageDir,imageName);
	//strcpy_s(output, (strlen(tempPath)+1)*sizeof(char), tempPath);
	sprintf(output, "%s\\%s", imageDir, imageName);
...
int CMuPDFConvert::getPageNumber(const wchar_t* wcharPdfFile, const char* imageDir, const char* imageName)
{
	int pagesNum = -1;
	char tempPath[1024];
	sprintf(output, "%s\\%s", imageDir, imageName);
...
bool CMuPDFConvert::getPngFromPage(int pageID)
{
	fz_page *page = fz_load_page(m_doc, pageID);
	fz_rect bound = fz_bound_page(m_doc, page);

	//PDF-->PNG
	drawpage(m_ctx, m_doc, pageID + 1);

	fz_free_page(m_doc, page);
	page = NULL;
	return true;
}
bool CMuPDFConvert::freeMemory()
{
	if (m_pStream)
	{
		fz_close(m_pStream);
	...
	if (output)
	{
		...
		fz_try(ctx)
		{
			...
			if (output)
			{
				...
				sprintf(buf, "%s%d.png", output, pagenum);
				if (strstr(buf, ".png"))
				{
